...
class SimpleRNNw2vClassifier(BaseRNNClassifier):
    """
    Simple RNN Classifier with pre-trained Word2Vec embeddings.
    """

    def __init__(self, config):
        """
        Initialize the SimpleRNNw2vClassifier.
    ...
    def build_model(self):
        """
        Build the architecture for the Simple RNN model with Word2Vec embeddings.
        """
        ...
        self.model = Sequential([
            ...
            Dense(units=1, activation=self.output_activation)
        ])
        self.model.compile(optimizer=self.optimizer, loss=self.loss, metrics=["accuracy"])
        print("Simple RNN w2v model built and compiled.")
