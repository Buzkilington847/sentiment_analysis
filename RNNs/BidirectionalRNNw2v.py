...
class BidirectionalRNNw2vClassifier(BaseRNNClassifier):
    """
    Bidirectional RNN Classifier with pre-trained Word2Vec embeddings.
    """

    def __init__(self, config):
        """
        Initialize the BidirectionalRNNw2vClassifier.

        Args:
            config (dict): Configuration dictionary.
        """
        super().__init__(config)
        self.config = config  # Explicitly assign the config attribute
        self.embedding_matrix = None  # Initialize the embedding matrix as None

    def build_model(self):
        """
        Build the architecture for the Bidirectional RNN model with Word2Vec embeddings.
        """
        ...
        self.model = Sequential([
            ...
            Bidirectional(
                SimpleRNN(units=self.rnn_units, activation=self.hidden_activation)
            ),
            # Dense output layer
            Dense(
        ...
        print("Bidirectional RNN w2v model built and compiled.")
